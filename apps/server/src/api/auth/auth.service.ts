...
export default class AuthService {
  ...
  /**
   * @method POST
   * @route /api/auth/revokeToken
   * @returns NOTHING
   */
  public async revokeToken(req: AuthenticatedRequest, res: Response) {
    const { revokeSession } = req.body;
    const { refreshToken } = req.cookies;
    if (!refreshToken) {
      throw new BadRequestError('No such session exists!');
    }

    // Revoke the token
    await RefreshToken.findOneAndUpdate({ rtoken: refreshToken }, { revoked: true, revokedAt: new Date() });
    ...
    return new SendSuccessResponse(res, 200, SuccessStatus.SUCCESS, {});
  }
